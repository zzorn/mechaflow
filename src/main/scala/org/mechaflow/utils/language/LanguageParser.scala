...
abstract class LanguageParser[T] extends StandardTokenParsers with PackratParsers with ImplicitConversions {
  ...
  /**
   * A parser for single quoted strings.
   */
  protected lazy val singleQuotedString: PackratParser[String] = accept("singleQuotedString", {
    case lexical.SingleQuotedStringLiteral(n) => n
  })
